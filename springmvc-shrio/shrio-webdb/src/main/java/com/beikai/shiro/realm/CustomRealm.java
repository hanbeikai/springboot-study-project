...
public class CustomRealm extends AuthorizingRealm {
    ...
    private Set<String> getRoleByUsername(String username) {
        // 从数据库中获取用户角色
        List<String> users = userDao.getRoleByUsername(username);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        simpleAuthenticationInfo.setCredentialsSalt(ByteSource.Util.bytes(getSaltByUsername(username)));
    ...
    private String getSaltByUsername(String username) {
        // 从数据库中通过用户名获取 salt
        User user = userDao.getSaltByUsername(username);
        if (user != null){
            return user.getSalt();
        }
        return null;
